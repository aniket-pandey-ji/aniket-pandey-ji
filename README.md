<h1 align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=28&pause=1000&color=58A6FF&center=true&vCenter=true&width=500&lines=Hey+%F0%9F%91%8B+I'm+Aniket+Pandey;Full-Stack+Developer;BCA+%40+DDU+Gorakhpur;Building+toward+FAANG" alt="Typing SVG" />
</h1>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=aniketpandey&label=Profile%20views&color=58a6ff&style=flat" alt="profile views" />
  <img src="https://img.shields.io/badge/Status-Open%20to%20Internships-brightgreen?style=flat" />
  <img src="https://img.shields.io/badge/Focus-Full--Stack%20%2B%20DSA-orange?style=flat" />
</p>

---

### 🧠 About Me

```js
const aniket = {
  education : "BCA @ MPPG College, DDU Gorakhpur (2024–2027)",
  stack     : ["Node.js", "Express", "MongoDB", "React", "C++", "JavaScript"],
  building  : "JARVIS — a full-stack AI life management system",
  goal      : "Tier-3 college → Product company (FAANG/Microsoft/Google)",
  devSetup  : "Samsung Tab A9 Plus + Termux + VS Code (yes, really)",
  currentFocus : ["DSA grind", "Backend architecture", "AI API integration"],
  motto     : "Ship → learn → repeat"
};
```

---

### 🚀 Projects

| Project | Description | Stack |
|---------|-------------|-------|
| **[JARVIS](https://github.com/aniketpandey)** | AI-powered personal life management system — chat, DSA log, task tracker, health & study monitoring | Node.js, Express, MongoDB, React, Claude API |
| **[Student Hub](https://github.com/aniketpandey)** | Student resource & community platform | Express, MongoDB, HTML, Vercel |
| **[AnikCode](https://github.com/aniketpandey)** | React-based code translation tool | React, JavaScript |
| **[Portfolio](https://github.com/aniketpandey)** | Personal portfolio website | HTML, CSS, JS |
| **[Joke Generator](https://github.com/aniketpandey)** | Claude API-powered joke app | Vanilla JS, Anthropic API |

---

### 🛠️ Tech Stack

<p align="left">
  <img src="https://skillicons.dev/icons?i=cpp,js,nodejs,express,mongodb,react,html,css,git,github,vscode,vercel,linux" />
</p>

**Learning next:** TypeScript · PostgreSQL · Docker · System Design

---

### 📊 GitHub Stats

<p align="center">
  <img width="48%" src="https://github-readme-stats.vercel.app/api?username=aniketpandey&show_icons=true&theme=github_dark&hide_border=true" />
  <img width="48%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=aniketpandey&layout=compact&theme=github_dark&hide_border=true" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=aniketpandey&theme=github-dark-blue&hide_border=true" />
</p>

---

### 📈 DSA Progress

```
LeetCode Journey:
[██████░░░░░░░░░░░░░░] Grinding daily — target: 300+ quality problems
Focus: Arrays → Recursion → OOP → Trees → DP
```

---

### 🔗 Connect

<p align="left">
  <a href="https://www.linkedin.com/in/aniket-pandey-74b800297" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://www.instagram.com/aniket_pandey07" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" />
  </a>
  <a href="mailto:aniketpandeyvimla@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>

---

<p align="center">
  <img src="https://raw.githubusercontent.com/aniketpandey/aniketpandey/output/snake.svg" alt="Snake animation" />
</p>

<p align="center">
  <i>"The only way out of Tier-3 is to outcode everyone who had better resources."</i>
</p>
